...
public interface ArticlesRepository extends JpaRepository<Articles, String> {
    ...
    @Query(value = "SELECT A.id, A.title, A.image_url AS imageUrl, A.body, A.created_at AS createdAt " +
    ...
    Page<PaginatedArticlesListIntrf> findAllArticles(@Param("title") String title, @Param("category") String category,
            Pageable pageable);

    Optional<Articles> findByBody(String body);
}
