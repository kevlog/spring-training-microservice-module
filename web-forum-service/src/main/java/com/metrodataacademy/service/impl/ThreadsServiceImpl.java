...
public class ThreadsServiceImpl implements ThreadsService {
    ...
    public ResponseEntity<ResBaseDto> getThreadsById(String id) {
        // TODO: implementasi logic Get Threads By Id
    ...
    public ResponseEntity<ResBaseDto> insertThreads(ReqCreateThreadsDto threadsDto, AuthorizationDto authDto) {
        // TODO: implementasi logic Insert Threads
        try {
    ...
    public ResponseEntity<ResBaseDto> getListThreads(ReqGetListThreads reqGetListThreads) {
        // TODO: implementasi logic Get List Threads
        try {
            Page<Threads> pageResult = Page.empty();
            Pageable pageable = PageRequest.of(reqGetListThreads.getPage() - 1, ConstantVariable.DATA_PER_PAGE_THREADS);

            if (reqGetListThreads.getFilterBy().equalsIgnoreCase("title")
                    && reqGetListThreads.getSortBy().equalsIgnoreCase("null")
                    || reqGetListThreads.getFilterBy().equalsIgnoreCase("title")
                            && reqGetListThreads.getSortBy().equalsIgnoreCase("D")) {
                pageResult = threadsRepository.findAllThreadsWithKeywordDesc(reqGetListThreads.getKeyword(), pageable);
            }

            if (reqGetListThreads.getFilterBy().equalsIgnoreCase("totalComments")
                    && reqGetListThreads.getSortBy().equalsIgnoreCase("D")
                    || reqGetListThreads.getFilterBy().equalsIgnoreCase("totalComments")
                            && reqGetListThreads.getSortBy().equalsIgnoreCase("")) {
                pageResult = threadsRepository.findAllThreadsByTotalCommentsDesc(pageable);
            }

            if (reqGetListThreads.getFilterBy().equalsIgnoreCase("totalComments")
                    && reqGetListThreads.getSortBy().equalsIgnoreCase("A")) {
                pageResult = threadsRepository.findAllThreadsByTotalCommentsAsc(pageable);
            }

            if (reqGetListThreads.getFilterBy().equalsIgnoreCase("totalViews")
                    && reqGetListThreads.getSortBy().equalsIgnoreCase("D")
                    || reqGetListThreads.getFilterBy().equalsIgnoreCase("totalViews")
                            && reqGetListThreads.getSortBy().equalsIgnoreCase("")) {
                pageResult = threadsRepository.findAllThreadsByTotalViewDesc(pageable);
            }

            if (reqGetListThreads.getFilterBy().equalsIgnoreCase("totalViews")
                    && reqGetListThreads.getSortBy().equalsIgnoreCase("A")) {
                pageResult = threadsRepository.findAllThreadsByTotalViewAsc(pageable);
            }

            if (reqGetListThreads.getFilterBy().equalsIgnoreCase("")
                    && reqGetListThreads.getSortBy().equalsIgnoreCase("D")) {
    ...
    public ResponseEntity<ResBaseDto> deleteThreads(String id) {
        // TODO: implementasi logic Delete Threads
        try {
