...
public class ArticlesServiceImpl implements ArticlesService {
    ...
    @Override
    public ResponseEntity<ResBaseDto> insertArticle(ReqCreateArticlesDto reqCreateArticlesDto,
            HttpServletRequest servletRequest) {
        // TODO: Implementasi logic Insert Article
        try {
    ...
    @Override
    public ResponseEntity<ResBaseDto> updateArticle(String id, ReqCreateArticlesDto reqCreateArticlesDto,
            AuthorizationDto authDto) {
        // TODO: Implementasi logic Update Article
        try {
            ...
            if (reqCreateArticlesDto.getCategories() != null) {
                ...
                if (articlesCategoriesList.size() != 0) {
                    ...
                    articlesCategoriesList.forEach(articlesCategories -> {
                        articlesCategoriesMap.put(articlesCategories.getCategories().getId(),
                                articlesCategories.getId());
                    });
    ...
    public ResponseEntity<ResBaseDto> getArticlesById(String id) {
        // TODO: Implementasi logic Get Article By Id
        ...
        List<ResCategoriesDto> categoriesDtolList = new ArrayList<>();
        ...
        articlesCategories.forEach(
                listCategory -> {
                    ...
                    categoriesDtolList.add(resCategoriesDto);
                });

        resDetailArticleDto.setCategories(categoriesDtolList);
        ...
        return new ResponseEntity<>(new ResBaseDto(resStagingUserDto, ConstantVariable.SUCCESS), HttpStatus.OK);
    }

    @Override
    public ResponseEntity<ResBaseDto> deleteArticle(String id) {
        // TODO: Implementasi logic Delete Article
        try {
    ...
    public ResponseEntity<ResBaseDto> getAllArticleList(ReqGetArticlesDto requestDetail) {
        // TODO: Implementasi logic Get Article List
        Pageable pageable = PageRequest.of(requestDetail.getPage() - 1, ConstantVariable.DATA_PER_PAGE);
        Page<PaginatedArticlesListIntrf> pageResult = articlesRepository.findAllArticles(requestDetail.getTitle(),
                requestDetail.getCategory(), pageable);

        List<ResArticlesDto> responseList = new ArrayList<>();
        pageResult.forEach(
                response -> {
                    ResArticlesDto articlesResponse = articlesMapper
                            .paginatedArticlesListIntrfToResArticlesDto(response);

                    List<ResCategoriesDto> categoriesDtoList = new ArrayList<>();

                    List<ArticlesCategories> articlesCategories = articlesCategoriesRepository
                            .findByArticles_Id(response.getId());

                    articlesCategories.forEach(
                            listCategory -> {
                                Categories categories = listCategory.getCategories();
                                ResCategoriesDto resCategoriesDto = categoriesMapper
                                        .categoriesToResCategoriesDto(categories);
